...
断断续续用了大半年读完这个时间读完这个,心情是极度复杂. ![](learn-C-in-21-Days/02.jpeg) .你tm一个章节举几个🌰就行了？还21天？是21X24小时才能读完书吧。然而读完书，总结还是要写的。(虽然代码还是不太懂，写不好算法)故只写部分现在工作和未来的工作可能用的代码。。。。
...
- 对指针执行`++`和`--`编译器会认为你要改变的是指向相邻的内存块地址，而不是相邻字节。

### 第廿六章：智能指针

- **智能指针:**包含运算符的类，其行为像常规指针，但智能指针能够及时妥善地销毁动态分配的数据，并实现了明确的对象生命周期（🤔️OC中ARC的基础？）。
- 是为了解决无法及时准确的释放动态分配对象内存而引起野指针问题。
- 指针的类型
  - 深复制（deep copy), 拷贝它指向的对象，生成一个新的个体。
